...
function getMessageSenderHtml(message) {
    var img = base_url + '/assets/images/no-user.jpg';
    if (message.from_user.meta_data.profile_image != null) {
        img = base_url + '/uploads/' + message.from_user.meta_data.profile_image;
    }
    return `
         ...
         <div class="col-md-10 col-10">
             <div class="messages msg_sent pb-2 text-right">
                 ...
                 <time datetime="${message.dateTimeStr}"> ${message.from_user.name} • ${message.dateHumanReadable} </time>
             </div>
         </div>
         <div class="col-md-2 col-2 avatar">
             <img src="` + img + `" width="50" height="50" class="img img-fluid">
         </div>
     </div>`;
}
...
function getMessageReceiverHtml(message) {
    ...
    return `
         ...
         <div class="col-md-10 col-10">
             <div class="messages msg_receive text-left pb-2">
                 <p class="mb-0">${message.content}</p>
                 <time datetime="${message.dateTimeStr}"> ${message.from_user.name}  • ${message.dateHumanReadable} </time>
             </div>
         </div>
...
function loadLatestMessages(container, user_id) {
    ...
    $.ajax({
        ...
        complete: function () {
            ...
            chat_area.animate({
                scrollTop: chat_area.outerHeight(true) * 1500
            }, 800, 'swing');
...
function send(to_user, message) {
    ...
    $.ajax({
        ...
        complete: function () {
            ...
            //                            chatBox.find(".chat-area").animate({ scrollTop: chat_area.outerHeight(true)*1200 }, 800, 'swing');
            chat_area.animate({
                scrollTop: chat_area.outerHeight(true) * 1500
            }, 800, 'swing');
...
function displayMessage(message) {
    ...
    if ($("#current_user").val() == message.from_user_id) {
        ...
        cloneChatBox(message.from_user_id, message.from_userName, function () {
            ...
            if (!chatBox.hasClass("chat-opened")) {
                ...
                chatBox.find(".chat-area").animate({
                    scrollTop: chatBox.find(".chat-area").outerHeight(true) * 1500
                }, 800, 'swing');
            } else {
