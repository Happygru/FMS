...
@endsection
@section('breadcrumb')
<li class="breadcrumb-item "><a href="{{ route('booking-quotation.index') }}">@lang('menu.bookings')</a></li>
              ...
              <div class="form-group">
                ...
                <select class="form-control" id="reservation">
                  @foreach($reservations as $reservation)
                    <option value="{{$reservation->id}}" @if($booking_detail->reservation_type == $reservation->id) selected @endif>{{ $reservation->name }}</option>
                  @endforeach
                </select>
              </div>
            </div>
...
<input type="hidden" value="{{$booking_detail->addon_id}}" id="active_addons">
@endsection
@section('script')
...
<script>
  ...
  var vehicles;
  var active_addons = JSON.parse($("#active_addons").val());
  ...
  function get_vehicles(id) {
    $.post("{{url('admin/fetch-vehicle-list-by-type')}}", { id, branch_id: $("#branch").val() }, function(res) {
      ...
      let str = '';
      vehicles.map((item) => {
        // str += `<option value="${item.vehicle_id}" ${item.vehicle_id == {{$booking_detail->vehicle_id}}}>${item.make_name}</option>`;
      })
  ...
  function save_booking() {
    ...
    $.ajax({
      url: "{{url('admin/booking-quotation-update')}}",
      type: "POST",
      data: postData,
